...
3. **PRD Gate** — Before the PRD proceeds to phasing, Hermes runs
   `autonomous-gate` in `prd` mode. It derives project-specific checks using
   `skills/autonomous-gate/references/prd-gate.md` and records evidence.
...
4. **Spec Kit (per phase)** — PreDocumentation runs the Spec Kit flow for
   each phase independently. Each phase produces one
...
- Consistency with the current app state is a hard requirement, not a
  preference.
- Spec Kit is mandatory for turning phases into specs and tasks. Setup:
  `specify init --here --force --integration codex --integration-options="--skills"`,
  then use the installed `speckit-*` skills per phase.
- Only DevOps deploys; staging comes before production, and production only
  after the Tester signs off.

## Checklists

`autonomous-gate` owns the gate methodology and mode prompts in
`skills/autonomous-gate/references/`. They are not static checklists. Hermes
selects `prd`, `spec`, `code`, or `ship`; the skill derives project-specific
checks and writes the evidence-backed decision beside the phase manifest.
...
- **Version control workflow.** One delivery branch per phase. Atomic commits
  ...
  touches `main`. Full strategy in `docs/framework/VERSION-CONTROL.md`.
...
```text
autonomous-harness/                # Framework source read by operator/Hermes
├── docs/framework/                # Roles, contracts, operations, templates
├── skills/                         # Installable agent bridge
│   └── autonomous-gate/references/ # Dynamic gate prompts

existing-product/                  # Product repository operated by Hermes
├── .agents/skills/                # Installed Harness and Spec Kit skills
├── .autonomous/                   # State, phases, evidence, retrospectives
├── .specify/                      # Spec Kit configuration
└── specs/001-feature-slug/        # spec.md, plan.md, tasks.md
```

**Artifact rules:**
- Every artifact is written before the next step starts.
- The review gate checks that artifacts exist and are complete.
- Test evidence is never summarized — raw screenshots and logs go in
  `.autonomous/evidence/`.
- Retrospectives are written within 24 hours of shipping the phase under
...
| **PRD with unanswered questions** | Agents start building with incomplete context; wrong assumptions compound | The PRD gate blocks progress until evidence resolves every critical question. No "TBD" survives. |
...
- **Husky:** Git hooks for pre-commit quality gates (lint, format, test, commit
  ...
  `.husky/pre-push`. See `docs/framework/VERSION-CONTROL.md` "Pre-Commit Gates."
