...
1. Ship gate passes with evidence recorded by `autonomous-gate`
...
The reviewer uses `autonomous-gate` in `code` mode to derive project-specific
checks. Core concerns include:
...
Spec Kit generates tasks.md
  → Operator runs `autonomous-gate` in `spec` mode
  → Gate passes
...
1. REVIEWER LOADS CONTEXT:
   ...
   - Reads the code gate decision and linked evidence
...
3. SHIP GATE (end of phase only):
   ship gate executed by `autonomous-gate`
...
5. DEPLOYMENT RECORD:
   Recorded in the ship gate decision:
